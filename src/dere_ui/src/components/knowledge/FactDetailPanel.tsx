          ...
          <div className="flex h-full flex-col gap-4 overflow-y-auto px-4 py-4 text-sm">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                {fact.valid_at && (
                  <Badge variant="outline">Valid from {new Date(fact.valid_at).toLocaleString()}</Badge>
                )}
                {fact.invalid_at && (
                  <Badge variant="outline">Invalid at {new Date(fact.invalid_at).toLocaleString()}</Badge>
                )}
                <Badge variant="secondary">Created {new Date(fact.created_at).toLocaleString()}</Badge>
              </div>
            </div>
