 ...
 */
export async function handleAddDependency(args: AddDependencyRequest): Promise<TaskWarriorTask> {
  console.log(`addDependency called with:`, args);

  try {
    // Use TaskWarrior modify command to add dependency
    const commandArgs = [args.task_uuid, "modify", `depends:${args.depends_on_uuid}`];
