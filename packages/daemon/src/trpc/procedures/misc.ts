...
function readNonEmptyString(value: unknown, fallback: string): string {
  ...
  return parsed && parsed.trim().length > 0 ? parsed : fallback;
}
...
export const ambientRouter = router({
  dashboard: publicProcedure
    .input(z.object({ limit_runs: z.number().optional(), limit_notifications: z.number().optional() }).optional())
    .query(async ({ input }) => {
      const limitRuns = input?.limit_runs ?? 8;
      const limitNotifications = input?.limit_notifications ?? 8;

      const config = await loadConfig();
      const ambientConfig = (config.ambient ?? {}) as Record<string, unknown>;
      ...
      const userId = readNonEmptyString(
        ambientConfig.user_id as unknown,
